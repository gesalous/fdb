...
#include "fdb5/parallax/ParallaxStore.h"
...
ParallaxStore::ParallaxStore(const Schema& schema, const Key& key, const Config& config) :
    Store(schema), ParallaxCommon(StoreRootManager(config).directory(key).directory_) {}

ParallaxStore::ParallaxStore(const Schema& schema, const eckit::URI& uri, const Config& config) :
...
std::unique_ptr<FieldLocation> ParallaxStore::archive(const Key& key, const void* data, eckit::Length length) {
    ...
    return std::unique_ptr<ParallaxFieldLocation>(new ParallaxFieldLocation(dataPath, position, length, Key()));
}
